...
void Raycast(const Eigen::Vector3d& start, const Eigen::Vector3d& end, const Eigen::Vector3d& min,
  ...
  //这段文字是关于一个快速的光线追踪算法的代码注释，源自1987年John Amanatides和Andrew Woo的论文《A Fast Voxel Traversal Algorithm for Ray Tracing》。
  //该算法的基础是对光线的参数化表示：origin + t * direction。在算法的遍历过程中，通过维护tMaxX，tMaxY和tMaxZ变量，
  //可以记录当光线沿着一个轴（即坐标的整数部分变化）越过一个立方体边界时，所需的最小步长所对应的t值。代码中还提到了一些对算法的扩展，例如距离限制和回调函数等。
  //最后，代码注释中还指出了起点所在的立方体。
...
// 这段代码实现了光线与立方体体素的相交检测，返回光线与立方体体素相交的点的坐标。其中传入参数包括光线起点、终点，立方体体素的最小和最大坐标，输出参数包括相交点的数量和相交点的坐标。
// 算法的核心思想是使用射线参数方程来表示光线，同时记录光线在经过不同轴上的每个立方体体素时距离起点的距离。
// 从起点开始遍历，依次判断当前点是否在立方体体素内部，如果是，则将该点添加到输出列表中，并计算该点与起点之间的距离。
// 如果该距离大于光线的最大距离，则停止遍历。如果当前点不在立方体体素内部，则根据记录的距离信息，选择射线在不同轴上经过立方体体素的点，然后更新当前点的坐标和距离信息。
// 最终，输出列表中包含了所有与光线相交的立方体体素的坐标。
