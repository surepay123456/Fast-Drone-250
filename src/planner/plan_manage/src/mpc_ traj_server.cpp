...
void MpcTrajServer::bsplineCallback(traj_utils::BsplineConstPtr msg) {
    ...
    this->convertBspline2Traj(traj_, 0.1, common_traj_now_);
    if (traj_id_ > 1) {
        // use deriavate to calculate the yaw
        // refinePerceptionYawTraj(common_traj_now_, common_traj_last_);
    }
    common_traj_last_ = common_traj_now_;
}
